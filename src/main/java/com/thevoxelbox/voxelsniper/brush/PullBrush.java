...
public class PullBrush extends Brush {
    ...
    private boolean isSurface(final int x, final int y, final int z) {
        return  !isAnyTypeOfAir(this.getBlockMaterialAt(x, y, z))
                && (isAnyTypeOfAir(this.getBlockMaterialAt(x, y - 1, z)))
                || (isAnyTypeOfAir(this.getBlockMaterialAt(x, y + 1, z)))
                || (isAnyTypeOfAir(this.getBlockMaterialAt(x + 1, y, z)))
                || (isAnyTypeOfAir(this.getBlockMaterialAt(x - 1, y, z)))
                || (isAnyTypeOfAir(this.getBlockMaterialAt(x, y, z + 1)))
                || (isAnyTypeOfAir(this.getBlockMaterialAt(x, y, z - 1)));
    ...
    private void setBlock(final BlockWrapper block) {
        ...
        if (isAnyTypeOfAir(this.getBlockMaterialAt(block.getX(), block.getY() - 1, block.getZ()))) {
            ...
            for (int z = -v.getBrushSize(); z <= v.getBrushSize(); z++) {
                ...
                for (int x = -v.getBrushSize(); x <= v.getBrushSize(); x++) {
                    ...
                    for (int y = v.getBrushSize(); y >= -v.getBrushSize(); y--) {
                        ...
                        if (volume <= brushSizeSquared && !isAnyTypeOfAir(this.getWorld().getBlockAt(actualX, this.getTargetBlock().getY() + y, actualZ).getType())) {
