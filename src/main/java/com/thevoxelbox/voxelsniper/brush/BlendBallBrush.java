            ...
            for (int y = 0; y <= brushSizeDoubled; y++) {
                for (int z = 0; z <= brushSizeDoubled; z++) {
                    ...
                    for (Entry<Material, Integer> e : materialFrequency.entrySet()) {
                        if (e.getValue() > highestMaterialCount && !(this.excludeAir && isAnyTypeOfAir(e.getKey())) && !(this.excludeWater && e.getKey() == Material.WATER)) {
            ...
            for (int y = 0; y <= brushSizeDoubled; y++) {
                ...
                for (int z = brushSizeDoubled; z >= 0; z--) {
                    if (xSquared + ySquared + Math.pow(z - brushSize - 1, 2) <= rSquared) {
                        if (!(this.excludeAir &&  isAnyTypeOfAir(newMaterials[x][y][z])) && !(this.excludeWater && (newMaterials[x][y][z] == Material.WATER))) {
