                ...
                for (int y = this.getTargetBlock().getY(); y > 0; y--) {
                    ...
                    if (memory[x + v.getBrushSize()][z + v.getBrushSize()] != 1) {
                        ...
                        if ((Math.pow(x, 2) + Math.pow(z, 2)) <= brushSizeSquared && splat[x + v.getBrushSize()][z + v.getBrushSize()] == 1) {
                            ...
                            if (isAnyTypeOfAir(check) || check == Material.WATER) {
                                    ...
                                    switch (this.getBlockMaterialAt(this.getTargetBlock().getX() + x, y, this.getTargetBlock().getZ() + z)) {
                                        ...
                                        case OBSIDIAN:
                                            ...
                                            for (int d = this.depth - 1; ((this.depth - d) <= depth); d--) {
                                                if (!isAnyTypeOfAir(this.clampY(this.getTargetBlock().getX() + x, y - d, this.getTargetBlock().getZ() + z).getType())) {
                ...
                for (int y = this.getTargetBlock().getY(); y > 0; y--) { // start scanning from the height you clicked at
                    if (memory[x + v.getBrushSize()][z + v.getBrushSize()] != 1) { // if haven't already found the surface in this column
                        if ((Math.pow(x, 2) + Math.pow(z, 2)) <= brushSizeSquared && splat[x + v.getBrushSize()][z + v.getBrushSize()] == 1) { // if inside of the column...&& if to be splattered
                            if (!isAnyTypeOfAir(this.getBlockMaterialAt(this.getTargetBlock().getX() + x, y - 1, this.getTargetBlock().getZ() + z))) { // if not a floating block (like one of Notch'world pools)
                                if (isAnyTypeOfAir(this.getBlockMaterialAt(this.getTargetBlock().getX() + x, y + 1, this.getTargetBlock().getZ() + z))) { // must start at surface... this prevents it filling stuff in if
                                    ...
                                    if (!this.allBlocks) { // if the override parameter has not been activated, go to the switch that filters out manmade stuff.
                                        ...
                                        final int depth = this.randomizeHeight ? generator.nextInt(this.depth) : this.depth;
                                        for (int d = 1; (d < depth + 1); d++) {
                                            this.currentPerformer.perform(this.clampY(this.getTargetBlock().getX() + x, y + d + yOffset, this.getTargetBlock().getZ() + z)); // fills down as many layers as you specify in
                                            // parameters
                                            memory[x + v.getBrushSize()][z + v.getBrushSize()] = 1; // stop it from checking any other blocks in this vertical 1x1 column.
                                        }
                                    }
